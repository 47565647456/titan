...
public class AccountQueryService
{
        ...
        try
        {
            ...
            while (await reader.ReadAsync())
            {
                try
                {
                    ...
                    _logger.LogDebug("Found grain: {GrainType} with ID {AccountId}", grainType, accountId);
    ...
    public async Task<bool> DeleteAccountAsync(Guid accountId)
    {
        try
        {
            ...
            await using var cmd = new NpgsqlCommand(@"
                ...
                AND (graintypestring LIKE '%account%' 
                     OR graintypestring LIKE '%Account%'
                     OR graintypestring LIKE '%IAccountGrain%')
            ", conn);
    ...
    public async Task<bool> AccountExistsAsync(Guid accountId)
    {
        ...
        {
            _logger.LogError(ex, "Failed to check if account {AccountId} exists", accountId);
            throw;
        }
    }
